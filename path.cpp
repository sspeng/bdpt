...
void Path::init_alternatingRandomWalkFromPixel(const Scene* scene, Sampler* sampler,
	Path::RayItem& sdata, Path::RayItem& tdata,
	...
	/* Use a special sampling routine for the first two sensor vertices so that
	the resulting subpath passes through the specified pixel position */
	tdata.curDepth = tdata.curVertex->sampleSensor(scene,
		sampler, pixelPosition, e0, v1, e1, v2);
	sdata.curDepth = 0;

	if (tdata.curDepth >= 1) {
		sensorSubpath.append(e0, v1);
	}
	else {
		pool.release(e0);
		pool.release(v1);
...
void Path::o_alternatingRandomWalkFromPixel(const Scene* scene, Sampler* sampler,
	...
	while (sdata.curVertex || tdata.curVertex) {
		...
		if (sdata.curVertex && (sdata.curDepth < sdata.maxDepth || sdata.maxDepth == -1)) {
			PathVertex *succVertexS = pool.allocVertex();
			PathEdge *succEdgeS = pool.allocEdge();
			...
			if (sdata.curVertex->sampleNext(scene, sampler, sdata.predVertex,
				...
				sdata.predEdge = succEdgeS;
				sdata.curDepth++;
			}
			else {
				pool.release(succVertexS);
				pool.release(succEdgeS);
				sdata.curVertex = NULL;
			}
		}
		else {
			sdata.curVertex = NULL;
